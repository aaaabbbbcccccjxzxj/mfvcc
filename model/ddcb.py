...
class Asc(nn.Module):
    ...
    def __init__(self, in_channel, reduce_ratio=8) -> None:
        ...
        # self.convp3 = nn.Conv2d(in_channel // reduce_ratio * 2 + in_channel, in_channel // 2, 1)
        # self.convp4 = nn.Conv2d(in_channel // reduce_ratio * 3 + in_channel, in_channel // 2, 1)
        # self.convp5 = nn.Conv2d(in_channel // reduce_ratio * 4 + in_channel, in_channel, 1)
        #
    ...
    def forward(self, x):
        # x1 = self.act(self.convp1(x))
        # x1 = self.act(self.conv_3(x1))
        # x2 = torch.cat((x, x1), dim=1)
        # x2 = self.act(self.convp2(x2))
        # x2 = self.act(self.conv_5(x2))
        # x3 = torch.cat((x, x1, x2), dim=1)
        # x3 = self.act(self.convp3(x3))
        # x3 = self.act(self.conv_7(x3))
        # x4 = torch.cat((x, x1, x2, x3), dim=1)
        # x4 = self.act(self.convp4(x4))
        # x4 = self.act(self.conv_9(x4))
        # x_end = torch.cat((x, x1, x2, x3, x4), dim=1)
        # x_end = self.act(self.convp5(x_end))
...
class Atc(nn.Module):
    ...
    def __init__(self, in_channel, reduce_ratio=8) -> None:
        ...
        # self.convp1 = nn.Conv3d(in_channel, in_channel // 2, (1, 1, 1))
        # self.convp2 = nn.Conv3d(in_channel // reduce_ratio * 1 + in_channel, in_channel // 2, (1, 1, 1))
        # self.convp3 = nn.Conv3d(in_channel // reduce_ratio * 2 + in_channel, in_channel // 2, (1, 1, 1))
        # self.convp4 = nn.Conv3d(in_channel // reduce_ratio * 3 + in_channel, in_channel // 2, (1, 1, 1))
        # self.convp5 = nn.Conv3d(in_channel // reduce_ratio * 4 + in_channel, in_channel, (1, 1, 1))
        #
        # self.conv_3 = nn.Conv3d(in_channel // 2, in_channel // reduce_ratio, (3, 1, 1), dilation=1, padding='same')
        # self.conv_5 = nn.Conv3d(in_channel // 2, in_channel // reduce_ratio, (3, 1, 1), dilation=2, padding='same')
        # self.conv_7 = nn.Conv3d(in_channel // 2, in_channel // reduce_ratio, (4, 1, 1), dilation=2, padding='same')
...
class Ddcb(nn.Module):
    ...
    def __init__(self, in_channel, reduce_ratio) -> None:
        super().__init__()
        self.dsb = Asc(in_channel, reduce_ratio)
        self.dtb = Atc(in_channel, reduce_ratio)
